...
// carry credentials, environment-selected credential files, and the selected
// daemon remote token file. Two deliberate limits:
//
...
func credentialDenyReadPaths(policy Policy) []string {
	...
	return credentialDenyReadPathsForEnvironment(credentialPathEnvironment{
		...
		DaemonTokenFile:   selectedDaemonRemoteTokenFile(),
	}, policy.AllowRead)
}
...
func credentialDenyReadPathsIn(home string, googleCredentials string, daemonTokenFile string, allowRead []string) []string {
	return credentialDenyReadPathsForEnvironment(credentialPathEnvironment{
		...
		DaemonTokenFile:   daemonTokenFile,
	}, allowRead)
}
